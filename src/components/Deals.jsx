...
const Deals = ({ setPage }) => {
  const [timeLeft, setTimeLeft] = useState({
    days: 4, hours: 13, minutes: 34, seconds: 56
  });

  const deals = [
    { name: "Smart watches", discount: "-25%", image: watchImg, search: "Smart Watch" },
    { name: "Smartphones", discount: "-15%", image: laptopImg, search: "Smartphone" },
    { name: "Headphones", discount: "-40%", image: goproImg, search: "Headphones" },
    { name: "Laptops", discount: "-25%", image: headphonesImg, search: "Laptop" },
    { name: "Electric Kettle", discount: "-25%", image: canonImg, search: "Kettle" },
  ];

  useEffect(() => {
    const endKey = 'dealEndTime';
    if (!localStorage.getItem(endKey)) {
      const end = new Date();
      end.setDate(end.getDate() + 4);
      end.setHours(end.getHours() + 13);
      localStorage.setItem(endKey, end.toISOString());
    }

    const timer = setInterval(() => {
      const end = new Date(localStorage.getItem(endKey));
      ...
      <div className="flex-1 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 h-full">
        {deals.map((deal, index) => (
          <div
            ...
            onClick={() => setPage('listing', deal.search)}
          >
